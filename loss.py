...
class BCEDiceLoss(nn.Module):
    ...
    def __init__(self, alpha, beta, weight, device):
        ...
        self.alpha = alpha
        self.bce = nn.BCEWithLogitsLoss(weight=torch.tensor([weight])).to(device)
        self.beta = beta
        self.dice = DiceLoss()
...
class DiceCrossEntropyLoss(nn.Module):
    def __init__(self, weight=None, size_average=True,device="cpu"):
    ...
    def forward(self, predicted, target):
        ...
        # Combine both losses (you can adjust the weights here if needed)
...
class DiceBCELoss_2(nn.Module):
    ...
    def forward(self, predicted, target):
        ...
        bce_loss = nn.BCEWithLogitsLoss(pos_weight = torch.tensor([25.0]))(predicted, target).to(self.device)
